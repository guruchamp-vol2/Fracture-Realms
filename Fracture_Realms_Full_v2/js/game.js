// js/game.js — FULL FILE REPLACE (word for word)
...
class Platform extends Rect {
  ...
  draw(ctx) {
    if (this.dead) return;
    const t = clamp(this.stand / this.breakAt, 0, 1);
    ctx.fillStyle = `hsl(${lerp(200, 10, t)},60%,${lerp(60, 45, t)}%)`;
    ctx.fillRect(this.x, this.y, this.w, this.h);
    ctx.strokeStyle = `hsl(${lerp(210, 0, t)},80%,${lerp(40, 70, t)}%)`;
    ctx.lineWidth = 2;
    ctx.beginPath();
    for (let i = 0; i < 3; i++) {
      const px = this.x + this.w * (i + 1) / 4;
      ctx.moveTo(px, this.y);
      ctx.lineTo(px + Math.sin(t * 10 + i) * 8, this.y + this.h);
    }
    ctx.stroke();
  }
}
class Player {
  constructor(id, x, y, input) {
    ...
    this.w = 34; this.h = 48; this.onGround = false; this.jumps = 2;
    this.facing = 1; this.dashCD = 0; this.dashCharges = 1; this.maxDash = 1;
    this.hp = 100; this.iframes = 0; this.combo = 0; this.comboTimer = 0;
    this.grapple = null; this.lastAttack = ''; this.style = 'sword';
    this.input = input; this.alive = true;
    this.tint = id === 1 ? '#90caf9' : '#a5d6a7';
  }
  rect() { return new Rect(this.pos.x - this.w / 2, this.pos.y - this.h / 2, this.w, this.h); }
}
class Enemy {
  constructor(x, y, t = 'grunt') {
    this.pos = { x, y }; this.vel = { x: 0, y: 0 }; this.w = 36; this.h = 44;
    this.hp = t === 'brute' ? 70 : 40; this.onGround = false;
    this.blockMelee = 0; this.blockMagic = 0; this.cool = 0; this.type = t;
  }
  rect() { return new Rect(this.pos.x - this.w / 2, this.pos.y - this.h / 2, this.w, this.h); }
}
...
export class Game {
  constructor(canvas, opts, campaign, sm) {
    ...
    // Safe bindings (UI might be missing in some tests)
    ...
    this.players = [
      new Player(1, this.W * 0.5, this.H * 0.2, () => this.input.p1()),
      new Player(2, this.W * 0.5 + 40, this.H * 0.2, () => this.input.p2())
    ];
  ...
  togglePause(force) {
    ...
    const show = force === undefined ? this.ui.pause.classList.contains('hidden') : !force;
    this.paused = !show;
  ...
  renderUpgradePanel() {
    ...
    const buy = (u) => {
      ...
      this.owned = this.owned || new Set(); this.owned.add(u.id);
    ...
    for (const u of U) {
      const el = document.createElement('div'); el.className = 'upg';
      const owned = this.owned?.has?.(u.id);
      el.innerHTML =
        `<div>${owned ? 'Purchased: ' : ''}${u.desc}</div>
  ...
  collideWithPlatforms(ent, dt) {
    ...
    for (const p of this.platforms) {
      if (p.dead) continue; const pr = new Rect(p.x, p.y, p.w, p.h);
  ...
  doMelee(p) {
    p.lastAttack = 'melee'; const hb = this.meleeHitbox(p);
    for (const e of this.enemies) {
      if (e.hp <= 0) continue;
      if (this.rectsOverlap(hb, e.rect())) {
        const block = e.blockMelee > 0 ? 0.25 : 1;
        e.hp -= (e.type === 'brute' ? 12 : 16) * block; this.fxSlash(e.pos.x, e.pos.y);
        if (block < 1) this.log('Enemy blocked your melee!'); this.audio.hit();
      }
    }
    if (this.boss) {
      const b = this.boss;
      if (this.rectsOverlap(hb, b.rect())) {
        const airborne = !p.onGround; const mult = p.airDmg || 1; const dmg = airborne ? Math.round(14 * mult) : 0;
        if (dmg > 0) {
          b.hp -= dmg; this.fxSlash(b.pos.x, b.pos.y, true);
          if (!b.enraged && b.hp < b.maxHP * 0.5) { b.enraged = true; this.log('Boss enraged! Hazards intensify.', 'danger'); }
          this.audio.hit();
  ...
  shootMagic(p) {
    p.lastAttack = 'magic'; const speed = 540; const spread = 0.06; const dir = p.facing; const angle = dir > 0 ? 0 : Math.PI;
    this.bullets.push({ x: p.pos.x, y: p.pos.y - 10, vx: Math.cos(angle + rand(-spread, spread)) * speed, vy: Math.sin(angle + rand(-spread, spread)) * speed, life: 1.2, dmg: 10, from: p.id });
    p.combo = Math.min(999, p.combo + 1); p.comboTimer = 2.0; this.audio.pew();
  }
  dash(p) {
    if (p.dashCD <= 0 && p.dashCharges > 0) {
      const power = 600; p.vel.x += power * p.facing; p.dashCD = 0.8; p.dashCharges--;
  ...
  draw() {
    ...
    const g = ctx.createLinearGradient(0, 0, 0, H); g.addColorStop(0, '#0b0f17'); g.addColorStop(1, '#091322');
    ...
    // players (fixed: removed Python-style 'and'/'pass' lines)
    for (const p of this.players) {
      if (p.id === 2 && !p.enabled) continue;
